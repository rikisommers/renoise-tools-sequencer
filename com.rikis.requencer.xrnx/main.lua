...
local BUTTON_COLOR_ACTIVE = {255, 200, 0}    -- Yellow/orange (Renoise button active color)
local BUTTON_COLOR_INACTIVE = {80, 80, 80}   -- Gray (inactive state)
local BUTTON_COLOR_PLAY = {147, 245, 66}     -- Green (play state)
local BUTTON_COLOR_STOP = {245, 66, 93}      -- Red (stop state)
local BUTTON_COLOR_WARNING = {245, 194, 66}  -- Orange (warning state)

-- Row container styling
local ROW_BACKGROUND_COLOR = {191, 191, 191}    -- Darker background for rows
local ROW_PADDING = 4                        -- Padding around each row
...
-- Convert MIDI note value (0-119) to note string (e.g., "C4", "D#3")
...
-- Map note value to rotary percentage (wrapper for compatibility)
local function map_note_to_rotary(note_value, base_note_value)
  -- This is essentially the same as note_to_percentage
  return note_to_percentage(note_value, base_note_value or 48)
end
...
current_step = 1
is_playing = false
local is_syncing_pattern = false
...
local function toggle_note_row_visibility(row_index)
  ...
  if vb.views[toggle_id] then
    vb.views[toggle_id].color = not is_visible and BUTTON_COLOR_ACTIVE or BUTTON_COLOR_INACTIVE
  end
...
local function toggle_chord_track(row_index)
  ...
  if vb.views[toggle_id] then
    vb.views[toggle_id].color = not is_chord and BUTTON_COLOR_ACTIVE or BUTTON_COLOR_INACTIVE
  end
...
update_mute_button_color = function(row_index)
  ...
  if vb.views[button_id] then
    ...
    vb.views[button_id].color = is_muted and BUTTON_COLOR_ACTIVE or BUTTON_COLOR_INACTIVE
  end
end
...
local function create_step_indicators(steps)
  ...
  row:add_child(vb:text{width = cellSizeLg, text = "Instrument", align = "left"})
  row:add_child(vb:text{width = cellSize, text = "C", align = "left"})
  row:add_child(vb:text{width = cellSizeLg, text = "Chord", align = "left"})
...
local function sync_pattern_to_sequencer()
  ...
  local ok, err = pcall(function()
    ...
    for row_index = 1, #sequencer_data do
      local track_index = get_track_index_for_row(row_index)
      local first_note_found = false  -- Track if we found the first note for this row
      ...
      if track_index and track_index <= #song.tracks then
        ...
        if note_column and note_column.note_value < 121 then
          ...
          -- Update track note rotary with the first note found (as base reference)
          if not first_note_found then
            first_note_found = true
            sequencer_data[row_index].note_value = note_column.note_value
            ...
            if vb.views[track_note_rotary_id] then
              ...
              local track_rotary_value = map_note_to_rotary(note_column.note_value, base_note_value)
              vb.views[track_note_rotary_id].value = track_rotary_value
            end
          end
          
          -- Update UI button with note name
          ...
          if vb.views[note_rotary_id] then
            local base_note_value = sequencer_data[row_index].base_note_value or 48
            local rotary_value = map_note_to_rotary(note_column.note_value, base_note_value)
            vb.views[note_rotary_id].value = rotary_value
          end
          
          -- Loop this note through the entire pattern (every num_steps lines)
          for line_index = step + num_steps, pattern.number_of_lines, num_steps do
          ...
          print("Row " .. row_index .. " Step " .. step .. ": Found note " .. note_column.note_value .. " (" .. note_value_to_string(note_column.note_value) .. ") - looped through pattern")
          else
            -- No note - set step to Off and clear from entire pattern
            ...
            for line_index = step, pattern.number_of_lines, num_steps do
              ...
              for col = 1, 12 do
                ...
                if loop_note_column then
                  loop_note_column.note_value = 121  -- Empty note
  ...
  local row = vb:row{
    spacing = control_spacing,
      ...
      vb:button{
        ...
        width = cellSize,
        height = cellSize,
        color = BUTTON_COLOR_INACTIVE,  -- Gray when not chord track
        ...
        vb:rotary{
          ...
          if sequencer_data[row_index].step_notes then
            for step_index, old_step_note in pairs(sequencer_data[row_index].step_notes) do
              ...
              -- Update the step button text to show new note
              local button_id = "step_button_" .. tostring(row_index) .. "_" .. tostring(step_index)
          ...
          if not is_syncing_pattern then
            for s = 1, num_steps do
              if sequencer_data[row_index].step_states and sequencer_data[row_index].step_states[s] then
                local step_state = sequencer_data[row_index].step_states[s]
                ...
                if step_state == 1 then
                  ...
                  if sequencer_data[row_index].step_notes and sequencer_data[row_index].step_notes[s] then
                    ...
                    -- Update button text for steps using track note
                    local button_id = "step_button_" .. tostring(row_index) .. "_" .. tostring(s)
                    if vb.views[button_id] then
      ...
      vb:button{
        ...
        width = cellSize,
        height = cellSize,
        color = BUTTON_COLOR_INACTIVE,  -- Gray when hidden (default)
...
local function create_note_row(row_index, steps)
  ...
  -- Wrap with minimal vertical spacing for tight alignment
  local wrapper = vb:column{
    spacing = 1,
    note_row
  }
  
  -- Store reference to the wrapper column (not just the inner row)
  track_note_rows[row_index] = wrapper
  wrapper.visible = track_visibility[row_index].note_visible
...
local function create_volume_row(row_index, steps)
  ...
  -- Store reference to the wrapper column (not just the inner row)
  track_volume_rows[row_index] = wrapper
  wrapper.visible = track_visibility[row_index].volume_visible
...
local function create_delay_row(row_index, steps)
  ...
  -- Store reference to the wrapper column (not just the inner row)
  track_delay_rows[row_index] = wrapper
  wrapper.visible = track_visibility[row_index].delay_visible
  
  return wrapper
end

-- Helper function to wrap a complete sequencer row group with styled container
-- (padding, background, rounded corners)
local function create_styled_row_group(row_index, steps)
  local step_row = create_step_row(row_index, steps)
  local note_row = create_note_row(row_index, steps)
  local volume_row = create_volume_row(row_index, steps)
  local delay_row = create_delay_row(row_index, steps)
...
local function update_play_button()
...
function show_sequencer_dialog()
  ...
  local function update_step_count(new_steps)
    if step_grid_view and dialog_content then
      ...
      for r = 1, #sequencer_data do
        if sequencer_data[r] then
          step_grid_view:add_child(create_styled_row_group(r, new_steps))
        end
      end
  ...
  local controls_row = vb:row{
    ...
    vb:button{
      ...
      notifier = function()
        ...
        -- Create UI row with styled container
        step_grid_view:add_child(create_styled_row_group(new_row_index, num_steps))
        apply_global_note_constraints()
  ...
  for r = 1, #sequencer_data do
    if sequencer_data[r] then
      ...
      -- Create the row with styled container
      step_grid_view:add_child(create_styled_row_group(r, num_steps))
    end
  end
  ...
  -- Wrap indicators and grid with custom spacing
