...
local function create_note_row(row_index, steps)
  local note_row = vb:horizontal_aligner{
    ...
    vb:text{width = 40, text = ""},  -- Volume button space
    vb:text{width = 40, text = ""},  -- Save button space
  }
...
local function create_volume_row(row_index, steps)
  ...
  -- Add rotary dials for each step volume
  for s = 1, steps do
    ...
    if step_grid_view and dialog_content then
      ...
      for r = 1, num_rows do
        if sequencer_data[r] then
          ...
          -- Update UI controls visibility for each row
          for s = 1, 64 do -- Assume max 64 steps possible
            local checkbox_id = "checkbox_" .. tostring(r) .. "_" .. tostring(s)
            local rotary_id = "step_note_rotary_" .. tostring(r) .. "_" .. tostring(s)
            local volume_rotary_id = "step_volume_rotary_" .. tostring(r) .. "_" .. tostring(s)
            
            if vb.views[checkbox_id] then
              vb.views[checkbox_id].visible = (s <= new_steps)
            end
            if vb.views[rotary_id] then
              vb.views[rotary_id].visible = (s <= new_steps)
            end
            if vb.views[volume_rotary_id] then
              vb.views[volume_rotary_id].visible = (s <= new_steps)
            end
          end
        end
      end
  ...
  local controls_row = vb:row{
   ...
   vb:button{
      ...
      notifier = function()
        ...
        step_grid_view:add_child(create_step_row(new_row_index, 64))  -- Create max steps
        step_grid_view:add_child(create_note_row(new_row_index, 64))  -- Create max steps
        step_grid_view:add_child(create_volume_row(new_row_index, 64))  -- Create max steps
        ...
        -- Hide steps beyond current step count
        for s = 1, 64 do
          local checkbox_id = "checkbox_" .. tostring(new_row_index) .. "_" .. tostring(s)
          local rotary_id = "step_note_rotary_" .. tostring(new_row_index) .. "_" .. tostring(s)
          local volume_rotary_id = "step_volume_rotary_" .. tostring(new_row_index) .. "_" .. tostring(s)
          
          if vb.views[checkbox_id] then
            vb.views[checkbox_id].visible = (s <= num_steps)
          end
          if vb.views[rotary_id] then
            vb.views[rotary_id].visible = (s <= num_steps)
          end
          if vb.views[volume_rotary_id] then
            vb.views[volume_rotary_id].visible = (s <= num_steps)
          end
        end
      end
    },
