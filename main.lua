...
local patternLength = 16 --renoise.song().patterns[pattern].number_of_lines
local cellSize = 24
...
-- Helper function to get the track index for a given row index
local function get_track_index_for_row(row_index)
  local song = renoise.song()
  ...
  if sequencer_group_index then
    -- Tracks are created after the group, so row 1 = group_index + 1, row 2 = group_index + 2, etc.
    return sequencer_group_index + row_index
  else
    -- Fallback: if no group found, assume tracks start at index 1
    return row_index
  end
end
...
-- Sequencer settings
local num_steps = 16 --patternLength
local num_rows = 1
        ...
        notifier = function(value)
          ...
          for s = 1, num_steps do
            if sequencer_data[row_index].steps[s] then
              ...
              note_to_use = snap_to_scale(note_to_use, select(1, compute_note_range(base_note_value)), select(2, compute_note_range(base_note_value)))
...
local function create_note_row(row_index, steps)
  ...
  local note_row = vb:horizontal_aligner{
    ...
    vb:text{width = cellSizeLg, text = ""},  -- Save button space
  }
...
local function show_sequencer_dialog()
  ...
  -- Initialize first row if sequencer_data is empty
  if #sequencer_data == 0 then
    local song = renoise.song()
    
    -- Find or create the Sequencer group track
    local sequencer_group_index = nil
    for i, track in ipairs(song.tracks) do
      if track.type == renoise.Track.TRACK_TYPE_GROUP and track.name == "Sequencer" then
        sequencer_group_index = i
    ...
    if not sequencer_group_index then
      setup_default_track_group()
      sequencer_group_index = 1
    end
    
    -- Create the first track after the group if it doesn't exist
    local first_track_index = sequencer_group_index + 1
    if first_track_index > #song.tracks then
      song:insert_track_at(first_track_index)
      song:add_track_to_group(first_track_index, sequencer_group_index)
      print("Created first track " .. first_track_index .. " in Sequencer group")
    end
    
    -- Initialize sequencer data for first row
    sequencer_data[1] = {instrument = 1, note_value = 48, base_note_value = 48, steps = {}, step_notes = {}, step_volumes = {}}
    for s = 1, num_steps do
      sequencer_data[1].steps[s] = false
    end
    -- Create UI for the first row
    step_grid_view:add_child(create_step_row(1, num_steps))
    step_grid_view:add_child(create_note_row(1, num_steps))
    step_grid_view:add_child(create_volume_row(1, num_steps))
  end
  ...
  local controls_row = vb:row{
   ...
   vb:button{
      ...
      notifier = function()
        ...
        sequencer_data[new_row_index] = {instrument = 1, note_value = 48, base_note_value = 48, steps = {}, step_notes = {}, step_volumes = {}}
        for s = 1, num_steps do
  ...
  -- Base controls width: TD(24) + TN(24) + ND(24) + I(96) + V(40) + S(40) = 248
  -- Step controls width: num_steps * 24
  -- Total width with margins
  local base_controls_width = cellSize * 3 + cellSizeLg + 40 + 40  -- 248
